	...
	: QWidget(parent), serv(serv), searchIndex(0)
{
	...
	QObject::connect(this->watchListWindowButton, &QPushButton::clicked, this, &UserGUI::OpenWatchListWindow);
}
UserGUI::~UserGUI()
{
...
void UserGUI::buildGUI()
{
	...
	this->watchListWindowButton = new QPushButton{ "show watchlist in table" };
	...
	rightLayout->addWidget(this->watchListWindowButton, 2, 6);
...
void UserGUI::OpenWatchListWindow()
{
	vector<Tutorial> tutorials = this->serv.get_watchlist();
	this->watchListWindow->setTutorials(tutorials);
