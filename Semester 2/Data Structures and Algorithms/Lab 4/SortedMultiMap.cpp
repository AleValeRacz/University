...
void SortedMultiMap::add(TKey c, TValue v) {
	...
	else
	{
		...
		if (current == pos)
		{
			//if it should be the first on that position, move the old one to the first empty and put the new one on the pos (apparently it's not a necesary step)
...
void SortedMultiMap::resize() {
	...
	for (int i = 0; i < oldCap; i++)
	{
		if (this->elements[i].first != NULL_TKEY) 
		{
			...
			{
				//if the new position where we need to place the element is empty
				...
				while (newNext[currentPos] != -1)
					//for finding where to put it if instead, if the new pos is occupied
